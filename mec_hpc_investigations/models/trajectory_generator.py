...
class TrajectoryGenerator(object):
    ...
    def _tower_distance(self, curr_pos_x, cond, left, right):
        ...
        tower_distance = (cond).astype(curr_pos_x.dtype) * within_tower_distance + (~cond).astype(
            curr_pos_x.dtype) * out_of_tower_distance
    ...
    def _determine_cue(self, curr_pos_x, mode):
        ...
        tower_data["tower1"]["cond"] = (curr_pos_x >= tower_data["tower1"]["left"]) | (
                    curr_pos_x <= tower_data["tower1"]["right"])

        tower_data["tower2"] = {}
        tower_data["tower2"]["left"] = 0.65
        tower_data["tower2"]["right"] = 0.95
        tower_data["tower2"]["cond"] = (curr_pos_x >= tower_data["tower2"]["left"]) & (
                    curr_pos_x <= tower_data["tower2"]["right"])

        tower_data["tower3"] = {}
        tower_data["tower3"]["left"] = 1.50
        tower_data["tower3"]["right"] = 1.70
        tower_data["tower3"]["cond"] = (curr_pos_x >= tower_data["tower3"]["left"]) & (
                    curr_pos_x <= tower_data["tower3"]["right"])

        tower_data["tower4"] = {}
        tower_data["tower4"]["left"] = 2.37
        tower_data["tower4"]["right"] = 2.43
        tower_data["tower4"]["cond"] = (curr_pos_x >= tower_data["tower4"]["left"]) & (
                    curr_pos_x <= tower_data["tower4"]["right"])

        tower_data["tower5"] = {}
        tower_data["tower5"]["left"] = 3.15
        tower_data["tower5"]["right"] = 3.25
        tower_data["tower5"]["cond"] = (curr_pos_x >= tower_data["tower5"]["left"]) & (
                    curr_pos_x <= tower_data["tower5"]["right"])
        ...
        if mode == "upcoming":
            ...
            # batch x 5 matrix
            curr_cue = np.stack(
                [self._tower_distance(curr_pos_x=curr_pos_x, **tower_kwargs) for tower_name, tower_kwargs in
                 tower_data.items()], axis=-1)
    ...
    def check_cue_2d_disjoint(self, cue1, cue2):
        ...
        return ((cue1_max_x < cue2_min_x) or (cue1_min_x > cue2_max_x) or (cue1_max_y < cue2_min_y) or (
                    cue1_min_y > cue2_max_y))
        ...
        if self.vr1d:
            ...
            if self.cue_2d_input_kwargs is not None:
                ...
                if cue_extents is not None:
                    ...
                    assert (
                        all([self.check_cue_2d_disjoint(cue1=cue_pair[0], cue2=cue_pair[1]) for cue_pair in cue_pairs]))
                    ...
                    for e in range(batch_size):
                        ...
                        while num_completed < max_cues:
                            ...
                            cue_data = {"center_x": cue_center_x, "center_y": cue_center_y, "width": cue_width,
                                        "height": cue_height}
                            # finally, ensure that the current cue does not overlap with the ones already found
                            if (num_completed == 0) or (
                            all([self.check_cue_2d_disjoint(cue1=cue_data, cue2=curr_ep_cue) for curr_ep_cue in
                                 episode_cues])):
            ...
            # rewards if any (2d environment only)
                ...
                if self.reward_zone_prob > 0:
                    ...
                    if (self.reward_zone_min_x is None) or (self.reward_zone_max_x is None) \
                        ...
                        assert (self.reward_zone_size <= (reward_max_x - reward_min_x)) and (
                                    self.reward_zone_size <= (reward_max_y - reward_min_y))
                    ...
                    if self.reward_zone_navigate_timesteps is not None:
                        ...
                        goal_velocity_x = np.tile(np.expand_dims(goal_velocity_x, axis=-1),
                                                  reps=(1, self.reward_zone_navigate_timesteps + 2))
                        goal_velocity_y = np.tile(np.expand_dims(goal_velocity_y, axis=-1),
                                                  reps=(1, self.reward_zone_navigate_timesteps + 2))
                        ...
                        reward_zone_traj, reward_zone_position = self._trajectory_builder(
                            samples=(samples + 1) - (self.reward_zone_navigate_timesteps + 2),
                        ...
                        assert (
                            np.isclose(navigate_position[:, self.reward_zone_navigate_timesteps + 1, 0], goal_x).all())
                        assert (
                            np.isclose(navigate_position[:, self.reward_zone_navigate_timesteps + 1, 1], goal_y).all())

                        # fill in the remaining timesteps with random walk within the reward zone
                        navigate_position[:, self.reward_zone_navigate_timesteps + 2:, :] = reward_zone_position[:,
                                                                                            reward_zone_pos_start_idx:,
                                                                                            :]
                    ...
                    for t in range(samples + 2):
                        reward_x = np.logical_and((reward_zone_min_x <= position[:, t, 0]),
                                                  (position[:, t, 0] <= reward_zone_max_x))
                        reward_y = np.logical_and((reward_zone_min_y <= position[:, t, 1]),
