...
def set_env_dims(options):
    if (not hasattr(options, 'min_x')) or (not hasattr(options, 'max_x')):
        assert(hasattr(options, 'box_width_in_m') and (options.box_width_in_m is not None))
    ...
    if (not hasattr(options, 'min_y')) or (not hasattr(options, 'max_y')):
        assert(hasattr(options, 'box_height_in_m') and (options.box_height_in_m is not None))
...
def get_model_activations(dataset, model, cfg,
    ...
    _, _, g_dict, p, pos, inp = compute_ratemaps(model,
                                    ...
                                    return_all=True if ((return_all) or (model_pred_layer not in ["g", "pc"])) else False)
...
def load_cached_metric_scores(metric, layer, rnn_type, activation, mode, eval_arena_size=100, cv_type="elasticnet_max"):
    ...
    curr_scores = np.load(os.path.join(BASE_DIR_RESULTS, f"arena{eval_arena_size}_model_{metric}scores.npz"), allow_pickle=True)['arr_0'][()]
    ...
    if (layer == "g") and (model_key in curr_scores.keys()):
        ...
        curr_scores = np.load(os.path.join(CAITLIN2D_MODEL_BORDERGRID_RESULTS,
                              f"{rnn_type}_bestneuralpredlayer_bordergridscores_{suffix}.npz"), allow_pickle=True)['arr_0'][()][f"{rnn_type}_{activation}_{mode}"][layer][f"{metric}scores"]
    return curr_scores
...
def configure_options(save_dir=None,
    ...
    class Options:
        pass
    options = Options()
    options.banino_place_cell = banino_place_cell
    options.optimizer_class = optimizer_class
    options.clipvalue = clipvalue
    if Nhdc is not None:
        options.Nhdc = Nhdc
        options.hdc_concentration = hdc_concentration
    ...
    options.place_cell_identity = place_cell_identity # decode positions directly if true
    options.place_cell_predict = place_cell_predict
    if options.place_cell_predict:
        options.place_cell_identity = True
        options.num_pc_pred = num_pc_pred
        assert(options.num_pc_pred == Np)
        options.pc_k = pc_k
    ...
    options.Ng = Ng             # number of grid cells
    options.place_cell_rf = place_cell_rf  # width of place cell center tuning curve (m)
    options.surround_scale = 2    # if DoG, ratio of sigma2^2 to sigma1^2
    options.RNN_type = rnn_type
    if options.RNN_type.lower() == "baninornn":
        options.banino_rnn_type = banino_rnn_type
        options.banino_rnn_nunits = banino_rnn_nunits
        options.banino_dropout_rate = banino_dropout_rate
    ...
    options.DoG = DoG            # use difference of gaussians tuning curves
    options.periodic = False      # trajectories with periodic boundary conditions
    if (arena_size is None) and (not env_1d):
        options.box_width_in_m = 2.2       # width of training environment (meters)
        options.box_height_in_m = 2.2      # height of training environment (metrs)
    else:
        if dataset is None:
            if env_1d:
                # faster to load saved out packaged data
                # from mec_hpc_investigations.core.default_dirs import CAITLIN1D_VR_PACKAGED
