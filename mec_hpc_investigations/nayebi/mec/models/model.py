...
class UGRNNCell(Layer):
    ...
    def call(self, inputs, states):
        ...
        assert(input_dim == self.units) # otherwise elementwise multiply of g * prev_state in new_state update will fail 
...
class BaninoRNN(Model):
    def __init__(self, options, place_cells):
        ...
        self.encoder1 = Dense(self.rnn_nunits,
                              kernel_initializer=initializers.VarianceScaling(scale=1.0, mode="fan_in", distribution="truncated_normal"),
        ...
        if self.rnn_type.lower() == "lstm":
            ...
            self.RNN = tf.keras.layers.LSTM(self.rnn_nunits, return_sequences=True,
                                            activation=options.activation,
                                            kernel_initializer=initializers.VarianceScaling(scale=1.0, mode="fan_in", distribution="truncated_normal"),
                                            recurrent_initializer=initializers.VarianceScaling(scale=1.0, mode="fan_in", distribution="truncated_normal"),
                                            bias_initializer="zeros",
    ...
    def compute_loss(self, inputs, cell_outputs, pos,
        ...
        if hasattr(self, "hdc_decoder"):
            ...
            loss = tf.reduce_mean(self.loss_fun(pc_outputs, pc_preds) + tf.nn.softmax_cross_entropy_with_logits(labels=hdc_outputs,
                                                                                                                logits=hdc_preds))
