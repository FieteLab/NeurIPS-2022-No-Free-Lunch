...
def compute_ratemaps(model, options, res=20, n_avg=None, idxs=None, trajectory_seed=0, eval_batch_size=None, return_all=False):
    '''Compute spatial firing fields'''

    eval_options = copy.deepcopy(options)
    if eval_batch_size is None:
        eval_batch_size = eval_options.batch_size
    else:
        # for trajectory generator
        eval_options.batch_size = eval_batch_size

    if return_all:
        assert(hasattr(model, "g"))
        assert(hasattr(model, "dc"))
        if hasattr(model, "pre_g"):
            if hasattr(eval_options, "banino_rnn_nunits"):
                pre_g_nunits = eval_options.banino_rnn_nunits
            else:
                pre_g_nunits = eval_options.Ng
            pre_g = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, pre_g_nunits])
        dc = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, eval_options.Np])

    if hasattr(eval_options, "Nhdc"):
        trajectory_generator = TrajectoryGenerator(options=eval_options,
                                                   place_cells=PlaceCells(eval_options),
                                                   head_direction_cells=HeadDirectionCells(eval_options),
    ...
    if not n_avg:
        n_avg = 1000 // eval_options.sequence_length

    if not np.any(idxs):
        idxs = np.arange(eval_options.Ng)
    idxs = idxs[:eval_options.Ng]

    g = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, eval_options.Ng])
    if eval_options.place_cell_predict:
        assert(hasattr(model, "pc"))
        p = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, eval_options.num_pc_pred])
    else:
        p = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, eval_options.Np])
    pos = np.zeros([n_avg, eval_batch_size * eval_options.sequence_length, 2])
    inp = []

    activations = np.zeros([eval_options.Ng, res, res])
    ...
    for index in tqdm(range(n_avg), leave=False, desc='Computing ratemaps'):
        ...
        if isinstance(p_batch, dict):
            p_batch = p_batch["place_outputs"]
        # the element of the input tuple is velocity or cues depending on the eval_options passed in
        ...
        if hasattr(model, "g"): # means it is an RNN (not NMF)
            if return_all:
                if hasattr(model, "pre_g"):
                    ...
                    assert(pre_g_batch.shape[-1] == pre_g_nunits)
                    pre_g_batch = np.reshape(pre_g_batch, (-1, pre_g_nunits))
                    pre_g[index] = pre_g_batch

                g_batch = model.g(inputs)
                assert(g_batch.shape[-1] == eval_options.Ng)
                g_batch = np.reshape(tf.gather(g_batch, idxs, axis=-1), (-1, eval_options.Ng))
                g[index] = g_batch

                dc_batch = model.dc(inputs)
                if hasattr(model, "hdc_decoder"):
                    assert(len(dc_batch) == 2)
                    dc_batch = dc_batch[0]
                assert(dc_batch.shape[-1] == eval_options.Np)
                dc_batch = np.reshape(dc_batch, (-1, eval_options.Np))
                dc[index] = dc_batch
            else:
                g_batch = model.g(inputs)
                assert(g_batch.shape[-1] == eval_options.Ng)
        ...
        if eval_options.place_cell_predict:
            p_batch = np.reshape(model.pc(inputs), (-1, eval_options.num_pc_pred))
        else:
            p_batch = np.reshape(p_batch, (-1, eval_options.Np))
        p[index] = p_batch

        pos_batch = np.reshape(pos_batch, (-1, 2))
        pos[index] = pos_batch

        x_batch = (pos_batch[:,0] - eval_options.min_x) / (eval_options.max_x - eval_options.min_x) * res
        y_batch = (pos_batch[:,1] - eval_options.min_y) / (eval_options.max_y - eval_options.min_y) * res

        for i in range(eval_batch_size*eval_options.sequence_length):
            x = x_batch[i]
            y = y_batch[i]
    ...
    if return_all:
        g = g.reshape((-1, eval_options.Ng))
        dc = dc.reshape((-1, eval_options.Np))
        g_dict = {"g": g, "dc": dc}
        if hasattr(model, "pre_g"):
            pre_g = pre_g.reshape((-1, pre_g_nunits))
            g_dict["pre_g"] = pre_g
    else:
        g = g.reshape((-1, eval_options.Ng))
        g_dict = g
    if eval_options.place_cell_predict:
        p = p.reshape((-1, eval_options.num_pc_pred))
    else:
        p = p.reshape((-1, eval_options.Np))
    pos = pos.reshape((-1, 2))
    ...
    rate_map = activations.reshape((eval_options.Ng, -1))
