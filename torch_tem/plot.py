...
def plot_map(environment, values, ax=None, min_val=None, max_val=None, num_cols=100, location_cm='viridis',
    ...
    for i, location in enumerate(environment.locations):
        ...
        location_patches.append(plt.Rectangle((location['x'] - radius / 2, location['y'] - radius / 2), radius, radius,
                                              color=location_cm(int(plotvals[i]))) if shape == 'square'
    ...
    for location in environment.locations:
        ...
        if location['shiny']:
            ...
            location_patches.append(
                plt.Rectangle((location['x'] - radius / 2, location['y'] - radius / 2), radius, radius, linewidth=1,
                              facecolor='none', edgecolor=[1, 0, 0]) if shape == 'square'
                else plt.Circle((location['x'], location['y']), radius, linewidth=1, facecolor='none',
                                edgecolor=[1, 0, 0]))
            # Add patches to axes
...
def plot_walk(environment, walk, max_steps=None, n_steps=1, ax=None):
    ...
    for step_i in range(1, max_steps, n_steps):
        ...
        new_loc = np.array(
            [environment.locations[walk[step_i][0]['id']]['x'], environment.locations[walk[step_i][0]['id']]['y']])
...
def plot_cells(p, g, environment, plots_dir_path: str, n_f_ovc=0, columns=10):
        ...
        for f, loc_rates in enumerate(cells):
            ...
            for c in range(len(loc_rates[0])):
                ...
                plot_map(environment, np.array([loc_rates[l][c] for l in range(len(loc_rates))]), ax[row, col],
                         shape='square', radius=1 / np.sqrt(len(loc_rates)))

        plt.savefig(os.path.join(plots_dir_path,
                                 f'rate_maps_{names}.png'),
                    bbox_inches='tight',
                    dpi=300)
